...
\begin{tikzpicture}[font=\small,>=Latex,node distance=0.8cm,
  ...
  \draw[->] (start)--(stk);
  \draw[->] (stk)-- node[right,font=\small]{machine available} (assign);
  \draw[->] (assign)--(setup);
  \draw[->] (setup)--(brk1);
  \draw[->] (brk1)-- node[right,font=\small]{no} (proc);
  \draw[->] (proc)--(brk2);
  \draw[->] (brk2)-- node[right,font=\small]{no (operation complete)} (more);
  \draw[->] (more)-- node[right,font=\small]{no} (done);
  \draw[crepair,thick] (brk1.west) -- node[above,font=\small]{yes} (failhub);
  \draw[crepair,thick] (brk2.west) -| node[below,font=\small,pos=0.2]{yes} (failhub);
  \draw[->,crepair,thick] (failhub)
    -- node[left,font=\small,align=center,pos=0.55]{return to stocker\\(same op)} (stockerReturn)
    -- (stk.west);
  \draw[->] (more.east) -- ++(2.7,0) |- node[right,font=\small,pos=0.25]{yes (next op)} (stk.east);
\end{tikzpicture}
\end{document}
